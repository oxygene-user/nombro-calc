...
#endif
#define END_CHECK(msg) if (s >= end) {WARNING("Unexpected eof while " msg "(line: %i)", get_current_line(s)); return false;}
#define SKIP_SEPARATORS(additional_check) \
	...
	{
		string_type comment(start, (signed_t)(s-start));
		if (comment.length() >= 2)
		{
			tools::trim(comment);
	...
	default:
		WARNING("'=' or '{' expected (line: %i)", get_current_line(s));
		return true;
	}
...
template <typename TCHARACTER> static const std::basic_string<TCHARACTER> store_value(const std::basic_string<TCHARACTER> &value, bool allow_unquoted = true)
{
	...
	replace_all(r, CONST_STR_BUILD(TCHARACTER,"`"), CONST_STR_BUILD(TCHARACTER,"``"));
